...
//		System.out.println("infoInsert.sh :: loginUser : " + loginUser);
		...
		// 해당 회원의 배송지 정보를 조회
		ArrayList<AddressInfo> aiList = shippingService.selectListAddressInfo(loginUser.getmNo());
...
//		System.out.println("infoInsert.sh :: basicChk : " + basicChk);
		...
		// 추가할 배송지 정보 객체에 담기
		ai.setMemberNo(loginUser.getmNo());
		ai.setMemberName(loginUser.getmName());
		if(basicChk != null) {
//			System.out.println("basicChk : null 아님 : " + basicChk);
			ai.setStatus("Y");	// 기본배송지로 설정
			
			// 기본 배송지는 하나여야 하므로 나머지 배송지의 state를 'N'으로 변경
			int updateResult = shippingService.changeState(loginUser.getmNo());
			if(updateResult == 0) {
